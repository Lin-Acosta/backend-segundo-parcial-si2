...
from src.modules.emergencies.models import Evidencia, Incidente
from src.modules.ai.models import AnalisisIA

from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.orm import Session
from src.modules.ai.services import ai_service
...
def clasificar_gravedad_endpoint(datos: DatosGravedad, forzar_reanalisis: bool = False, db: Session = Depends(get_db)):
    ...
    analisis_record = db.query(AnalisisIA).filter(AnalisisIA.incidente_id == datos.incidente_id).first()
    ...
    try:
        ...
        if not analisis_record:
            analisis_record = AnalisisIA(incidente_id=datos.incidente_id, NivelPrioridad=gravedad_str)
            db.add(analisis_record)
        else:
