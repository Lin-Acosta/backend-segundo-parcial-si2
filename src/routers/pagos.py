...
def pago_directo(
    ...
    pago_previo = db.query(models.Pago).filter(
        models.Pago.incidente_id == incidente.id,
        models.Pago.estado.in_(["Completado", "Pendiente Confirmación"])
    ).first()
    if pago_previo:
...
@router.post("/{incidente_id}/confirmar-directo", response_model=schemas.PagoOut)
def confirmar_pago_directo(
    ...
    taller = db.query(models.Taller).filter(
        models.Taller.Id == incidente.taller_id,
    ...
    pago = db.query(models.Pago).filter(
        ...
        models.Pago.estado == "Pendiente Confirmación",
        models.Pago.metodo == "Directo"
    ).first()
