...
def mis_incidentes(
    ...
    incidentes = (
        ...
        .options(
            ...
            joinedload(models.Incidente.analisis_ia),
            joinedload(models.Incidente.cotizaciones).joinedload(models.Cotizacion.taller),
            joinedload(models.Incidente.pagos)
        )
...
def talleres_disponibles(
    ...
    if incidente_id:
        incidente = db.query(models.Incidente).options(jl(models.Incidente.analisis_ia)).filter(models.Incidente.id == incidente_id).first()
...
def mantenimientos_taller(
    ...
    query = db.query(models.Incidente).options(
        ...
        models.Incidente.estado.in_(["Asignado", "En Camino", "Resuelto", "Pagado"])
    )
...
def _get_nombre_y_rol(usuario: models.Usuario):
...
def _usuario_puede_chatear(incidente: models.Incidente, user: models.Usuario) -> bool:
...
@router.get("/{incidente_id}/chat", response_model=List[schemas.MensajeChatOut])
def obtener_chat(
    incidente_id: int,
    db: Session = Depends(get_db),
    current_user: models.Usuario = Depends(get_current_user)
    ...
    incidente = db.query(models.Incidente).options(
        joinedload(models.Incidente.mecanicos)
    ).filter(models.Incidente.id == incidente_id).first()
    ...
    mensajes = (
        ...
        .options(joinedload(models.MensajeChat.usuario).joinedload(models.Usuario.rol))
        .filter(models.MensajeChat.incidente_id == incidente_id)
        .order_by(models.MensajeChat.id.asc())
        .all()
    )
    ...
    for m in mensajes:
        ...
        resultado.append(schemas.MensajeChatOut(
            id=m.id,
...
@router.post("/{incidente_id}/chat", response_model=schemas.MensajeChatOut)
def enviar_mensaje_chat(
    incidente_id: int,
    payload: schemas.MensajeChatCreate,
    db: Session = Depends(get_db),
    current_user: models.Usuario = Depends(get_current_user)
):
